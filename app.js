...
/* 霓虹脈衝 */
@keyframes neonPulse { 
  0%,100% { box-shadow: 0 0 10px rgba(0,230,255,.35), inset 0 0 10px rgba(0,230,255,.12) }
  50%     { box-shadow: 0 0 18px rgba(0,230,255,.55), inset 0 0 14px rgba(0,230,255,.18) }
}
.nav-tab.active { animation: neonPulse 2.2s ease-in-out infinite }
...
.page.active { animation: fadeUp .36s ease both }

/* 卡片進場（父容器加 .reveal-stagger 後生效） */
.reveal-stagger .army-card { 
  opacity:0; transform: translateY(12px); 
  animation: fadeUp .44s ease both 
}
.reveal-stagger .army-card:nth-child(1){ animation-delay:.08s }
.reveal-stagger .army-card:nth-child(2){ animation-delay:.14s }
.reveal-stagger .army-card:nth-child(3){ animation-delay:.20s }
.reveal-stagger .army-card:nth-child(4){ animation-delay:.26s }

/* 卡片霓虹描邊 */
@keyframes glowBorder {
  0%   { border-color: rgba(124,92,255,.25) }
  50%  { border-color: rgba(0,230,255,.45) }
  100% { border-color: rgba(124,92,255,.25) }
}
.army-card { animation: glowBorder 4s linear infinite }

/* 按鈕微互動 */
.story-toggle-btn, .nav-tab {
  position: relative; overflow: hidden;
}
.story-toggle-btn:after, .nav-tab:after{
  content:""; position:absolute; left:50%; top:50%; width:0; height:0;
  background: radial-gradient( rgba(255,255,255,.25), transparent 60%);
  transform: translate(-50%,-50%); transition: width .35s ease, height .35s ease; pointer-events:none;
}
.story-toggle-btn:active:after, .nav-tab:active:after{ width:160px; height:160px }

/* 打字機容器更像光框 */
.army-story{
  border-image: linear-gradient(90deg, rgba(0,230,255,.45), rgba(124,92,255,.45)) 1;
  }
